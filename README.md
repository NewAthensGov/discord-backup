...
9. Download these files by clicking the green Code button above, then Download ZIP,
10. Extract the files from the zip into a folder.
11. Add the name of the target server to the `let guild =` variable in `backup.js`.
12. Open command prompt or terminal and navigate to the folder that holds these scripts using this command: `cd path/to/folder`.
13. Run the following command in command prompt/terminal: `node backup.js` to create your first backup.
14. To restore from a backup, add the secret bot token and the name of the server you'd like to overwrite into the `restore.js` file and repeat step 10 and 11, but with `restore.js` instead of the backup script.
